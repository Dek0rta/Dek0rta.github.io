    ...
    <article className={`path__row ${m.live ? "path__row--live" : ""}`} data-reveal>
      ...
      <div className="path__body">
        <h3 className="path__title display">
          <span className="path__year-sr">{m.year} — </span>
          {m.title}
        </h3>
        <p className="path__text">{m.body}</p>
    ...
    <section className="section path" id="path">
      <div className="wrap">
        <div className="folio" data-reveal>
          ...
          <span className="folio__label">The path</span>
          <ProofMark k="path" />
        </div>

        <p className="path__lead display" data-reveal>
          No bootcamp, no CS class. Just five years of shipping — here&apos;s the
          record, in order.
        </p>

        <div className="path__list">
          {path.map((m, i) => (
            <Milestone key={m.year + m.title} m={m} i={i} />
          ))}
        </div>
      </div>
    </section>
